...
const myArray: Array<number | string | boolean> = [1, 2, 3, 4, 'yes', 3, 9, 'no', true];

// Generate a random start index within the array length
const startIndex: number = Math.floor(Math.random() * myArray.length);

// Iterate over the array starting from the random index
for (let i: number = startIndex; i < myArray.length; i++) {
